...
void ValSPMC(char* infile, Double_t MEAN, Double_t VAR)
{
  ...
  string xtitle="Inferred Positron Beam Energy, E_{+} [GeV]";
  ...
  Int_t k = t->Draw(Form("0.5*(DiMup4->E() - DiMup4->Pz())>>hBetaSP(90,%f,%f)",XMIN,XMAX),Form("0.5*(DiMup4->E() - DiMup4->Pz()) > %f && 0.5*(DiMup4->E() - DiMup4->Pz()) < %f",XMIN,XMAX),""); //USE THIS IF GP2X
  ...
  // Plot data
  RooPlot* frame=x.frame(Title("Generator Level Inferred E_{+}"));
  db.plotOn(frame);
  ...
  frame->SetTitle("Generator Level Inferred E_{+}");
...
void ValSPDET(char* infile, Double_t MEAN, Double_t VAR)
{
  ...
  // Plot data
  RooPlot* frame=x.frame(Title("Deconvolved Detector Level Inferred E_{+}"));
  db.plotOn(frame);
  ...
  frame->SetMaximum(ymaxh);
  frame->SetMinimum(yminh);
  frame->SetTitle("Deconvolved Detector Level Inferred E_{+}");
  frame->GetXaxis()->SetTitle(xtitle.c_str());      // Use command line argument for this
  ...
  frame3->Draw();
  //hres2->Draw("hist e2");
  c->SaveAs("DETspBetaEp.pdf");
  f->Close();
}

void DeconValP(char *infile, char *infileDECON)
{
  ...
  ValSPDET(infileDECON,125.0,0.1875);
  //Pass false to use the normal data settings
  ValSPMC(infile,125.0,0.1875);
}
