...
void ValSPMC(char* infile, Double_t MEAN, Double_t VAR)
{
  ...
  string xtitle="Inferred Electron Beam Energy, E_{-} [GeV]";
...
void ValSPDET(char* infile, Double_t MEAN, Double_t VAR)
{
  ...
  frame3->Draw();
  //hres2->Draw("hist e2");
  c->SaveAs("DETspBetaEm.pdf");
  f->Close();
}

void DeconValM(char *infile, char *infileDECON)
{
  ...
  ValSPDET(infileDECON,125.0,0.2375);
  //Pass false to use the normal data settings
  ValSPMC(infile,125.0,0.2375);
}
