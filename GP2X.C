...
//Second declaration of function in the case where the user does not provide the beam energy spread values
void GP2X(Int_t ngen, char *fname, char *gpname, char *kkname)
{
  //Set precision of print statements
  cout.precision(17);
  //Setup the GP input file
  ...
  FITVALS.PrintValues();
  Double_t BES1, BES2;
  BES1 = FITVALS.ENOM1*0.01;
  BES2 = FITVALS.ENOM2*0.01;
  cout << "USER DID NOT PROVIDE BEAM ENERGY SPREAD" << endl;
  cout << "WE ASSUME 1% BEAM ENERGY SPREAD" << endl;
  cout << "BEAM ENERGY SPREAD E1 [GeV] : " << BES1 << endl;
  cout << "BEAM ENERGY SPREAD E2 [GeV] : " << BES2 << endl;
  //FOR THE CROSS-SECTION FROM xsec:sqs
  Double_t XSEC_MAX = FITVALS.SX2*(FITVALS.ELO-7.0*FITVALS.DELENE)*(FITVALS.ELO-7.0*FITVALS.DELENE) + FITVALS.SX1*(FITVALS.ELO-7.0*FITVALS.DELENE) + FITVALS.SX0;
  //THIS IS THE COUNT OF HOW MANY EVENTS YOU'VE MADE
  //IT IS IN CAPS BECAUSE IT IS IMPORTANT
